...
magick -size 100x100 xc:black "%TEMP%\black_tile.png"
...
for /l %%y in (%rows%,-1,1) do (
    for /l %%x in (1,1,%columns%) do (
        ...
        if exist "%input_dir%!padded_index!.png" (
            set "montage_cmd=!montage_cmd! "%input_dir%!padded_index!.png""
        ) else (
            echo Warning: %input_dir%!padded_index!.png not found. Using black tile instead.
            set "montage_cmd=!montage_cmd! "%TEMP%\black_tile.png""
        )
    )
)
